...
def main(args, resume_preempt=False):
    ...
    use_vit_pred = cfgs_model.get('use_vit_pred', False)
    ...
    try:
        tensorboard_writer = SummaryWriter(log_dir=os.path.join(folder, 'tensorboard'))
    except Exception as e:
        logger.info(f'TensorBoard writer disabled: {e}')
        tensorboard_writer = NoOpSummaryWriter()
        ...
        for itr in range(ipe * accum_steps):
            ...
            def log_stats():
                ...
                if ((itr/accum_steps) % log_freq == 0) or np.isnan(loss) or np.isinf(loss):
                    ...
                    tensorboard_writer.add_scalar('Stats/TargetVar', target_var_meter.avg, epoch*ipe+itr)
                    tensorboard_writer.add_scalar('Stats/PredVar', pred_var_meter.avg, epoch*ipe+itr)
                    tensorboard_writer.add_scalar('Stats/ClipsPerSec', throughput_meter.avg, epoch*ipe+itr)
                    tensorboard_writer.add_scalar('Stats/DeviceHours', device_hours_meter.sum, epoch*ipe+itr)
                    tensorboard_writer.add_scalar('Time/GPU', gpu_time_meter.avg, epoch*ipe+itr)
                    tensorboard_writer.add_scalar('Memory/GPU', memory_mb, epoch*ipe+itr)
